...
% Put a PNG at graphics/ObsidianSeal.png or change path below.
\AddToHook{shipout/background}{%
  \ifwatermark
    \begin{tikzpicture}[remember picture,overlay]
      \node[opacity=0.06] at (current page.center)
        {\includegraphics[width=0.6\paperwidth]{1_OBSIDIAN_WALL/obsidian.sigils/OBSIDIANzed.png}};
    \end{tikzpicture}%
  \fi
}
...
% ===========================
% Release-only overrides
% ===========================

% PrivateBlock → always hidden in release append mode
\RenewDocumentEnvironment{PrivateBlock}{ O{security} O{} }
  { \begin{comment} }{ \end{comment} }

% Watermark defaults ON in this override
\watermarktrue

% Ritual/Whisper keep release framing
% (already defined above, but we can leave them here
%   as identity definitions for clarity)
\RenewDocumentEnvironment{ritual}{}%
  { \begin{tcolorbox}[gnomebox,title=Ritual] }{ \end{tcolorbox} }
\RenewDocumentEnvironment{whisper}{}%
  { \begin{tcolorbox}[gnomebox,title=Whisper] }{ \end{tcolorbox} }

% Resonance index stays as-is from main file.
