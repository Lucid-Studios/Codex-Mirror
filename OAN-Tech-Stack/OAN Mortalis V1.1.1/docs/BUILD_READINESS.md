...
- the master-workspace asset-class constitution now lives in
  `docs/governance/MASTER_WORKSPACE_ASSET_CLASS_CONSTITUTION.md`
...
- `Lab SaaS Assets/` is now the master-workspace source/formation surface,
  while this line-local `build/` folder remains the active verification and
  executable truth surface
- this line may consume bounded derived inputs, but it must not claim direct
  `RootAtlas` residency or raw lab-side templating authority
